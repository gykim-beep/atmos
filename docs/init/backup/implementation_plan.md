...
- **핵심 크레이트(Crate) 추천**:
  - `cpal`: Windows(ASIO) 및 macOS(Core Audio) 드라이버를 직접 스캔하고, 선점형으로 오디오 디바이스 스트림을 가동하는 초저지연 오디오 최적화 크레이트.
  - `symphonia`: 고성능 무부하 MP3 및 오디오 포맷 디코딩 처리.
...
- **도구**: `flutter_rust_bridge` (v2)
  ...
  - 복잡한 C++ FFI(Foreign Function Interface) 보일러플레이트 코드를 직접 짤 필요가 없어 개발 생산성과 안정성이 대폭 향상됩니다.
...
본 하이브리드 아키텍처의 안정성과 성능을 입증하기 위해 다음과 같은 로컬 오프라인 테스트 절차를 수행합니다.

### 1) 로컬 오프라인 테스트 방법
1. **Flutter 로컬 구동**: `flutter run -d macos` (또는 `windows`) 명령어를 사용해 독립형 Native 데스크톱 윈도우가 가볍게 구동되는지 확인합니다.
2. **OSC 모의 시뮬레이터**: 로컬 루프백(`127.0.0.1:8000`) 주소로 Python 스크립트나 간단한 OSC 전송 툴을 사용해 패킷을 전송했을 때, Rust 오디오 엔진이 딜레이 없이 소리를 부드럽게 페이드인/아웃 시키는지 확인합니다.
3. **리소스 모니터링**: macOS 활성 상태 보기(Activity Monitor) 또는 Windows 작업 관리자를 통해 다수의 BGM/SFX 재생 시 기존 대비 CPU 연산 소요 시간(SIMD 가속 확인)과 RAM 점유율(이중 링버퍼 확인)이 획기적으로 낮아졌는지 점검합니다.

---

## 10. 🚨 각 문서별 복원 및 추가 수정 상세 명세 (Nanoscopic Review)

*본 항목은 7개의 개별 문서(`1_PRD` ~ `7_CODING_CONVENTION`)로 분할하면서 도출된 각 세부 문서들의 누락 복구 및 아키텍처 강화(Atomic, 핫플러그, 오더링) 내역을 명세합니다.*

### [1_PRD_Product_Requirements_Document.md]
**복구(누락된 원본) 및 강화 사항:**
1. **스케일 및 하드웨어 타겟 명시 (복구)**: 최대 200개 이상의 개별 오디오 에셋과 돌비 애트모스 7.1.4 규격(12ch)의 동시 다발적 믹싱을 견디며, `RME Digiface USB` 및 `ADA8200` ADAT 컨버터 하드웨어를 직접 제어한다는 물리적 제품 목표 복원.
2. **네트워크 회복 탄력성 (강화)**: 오프라인 환경에서 네트워크 허브 전원이 차단되었다 복구되더라도 데스크톱 앱 재시작 없이 UDP `0.0.0.0:8000` 바인딩이 **지수 백오프(Exponential Backoff)**로 자동 복구되는 메커니즘 추가.
3. **비상 정지(Panic) 하드 스탑 (강화)**: 긴급 상황 시 페이드 아웃 없이 즉각적인 0ms 하드 스탑 연산 및 전 룸 `Locked` 초기화 구동.

### [2_TRD_Technical_Requirements_Document.md]
**복구(누락된 원본) 및 강화 사항:**
1. **스레드 통신 모델 아키텍처 (복구)**: `Mermaid`를 활용한 "스레드 철의 장막" 다이어그램 (OSC Listener -> OscDispatcher -> Lock-free MPSC -> Global Queue -> CPAL Real-time 믹서) 복원.
2. **수학적 OOM 임계치 증명 (복구)**: SFX 캐싱(약 345MB)의 안전성과 BGM 전체 캐싱(최대 6.9GB)의 위험성을 대조한 수학적 메모리 추정치와, 96kHz 스트리밍 시 디스크 대역폭(17.28 MB/s)이 SSD의 0.5% 미만이라는 물리적 병목 검증 수식 복원.
3. **위상 정렬 및 Object Pool (복구)**: 돌비 멀티트랙의 샘플 어긋남(Comb-Filtering)을 막는 단일 `SyncPlayhead` 공유 설계와, 동적 할당을 피하기 위한 `SoundInstance` 64개 한도 Object Pool 재사용 설계 복원.
4. **리샘플링 및 다운믹스 수학 공식 (복구)**: 선행(Pre) 리샘플링 vs I/O 스레드 슬라이딩 리샘플링 아키텍처, 선형 보간 수식($Sample_{dst} = (1.0-\alpha)Sample_1 + \alpha Sample_2$), 채널 모듈로 매핑 수식($Ch_{physical} = Ch_{target} \pmod{Max}$), $tanh$ 소프트 클리핑 수식 복원.
5. **다중 네트워크 인터페이스 (복구)**: UDP(유/무선) 외에 USB 시리얼 포트(9600bps) 직접 연결을 통한 OSC 파싱 변환 기능 복원.
6. **안전성 최적화 (강화)**: 락프리 링버퍼 Head/Tail 조작 시 `Acquire/Release` 메모리 오더링 강제, Flutter UI 스레드 과부하 방지를 위한 FFI 상태 업데이트 16.6ms Batching 전송.

### [3_USER_FLOW_Diagrams.md]
**복구(누락된 원본) 및 강화 사항:**
1. **라우팅 및 환경설정 플로우 (복구)**: 환경설정 팝업에서 하드웨어 스캔, 버퍼 사이즈 조절, 12x24 라우팅 매트릭스 그리드를 통해 채널 스왑을 시각적으로 수행하는 흐름 복원.
2. **하드웨어 핫-언플러그 플로우 (강화)**: 라이브 진행 중 USB 오디오 카드 전원 탈락 시 OS 기본 장치로 즉각 Fallback하고 UI에 붉은색 에러 배너를 띄우는 예외 흐름 추가.
3. **휴먼 에러 방지 (강화)**: 룸/트랙 삭제 시 휴먼 에러를 방지하는 "정말 삭제하시겠습니까?" 컨펌 다이얼로그 플로우 추가.

### [4_DB_DESIGN_Schema.md]
**복구(누락된 원본) 및 강화 사항:**
1. **정밀 오디오 스키마 (복구)**: `tracks` 스키마 내부에 빠져있던 `fade_in_sec`, `fade_out_sec` 개별 페이딩 설정 필드 복원.
2. **데이터 원자적 쓰기 (Atomic Write) (강화)**: 갑작스러운 PC 전원 차단 시 `config.json` 증발(Corrupted)을 100% 방어하기 위한 `tmp -> Rename` 시스템 콜 적용 설계.
3. **버전 관리 (강화)**: 스키마의 하위 호환성을 위한 `"schema_version"` 최상위 필드 도입.

### [5_SCREENS_UXUI_Design.md]
**복구(누락된 원본) 및 강화 사항:**
1. **정확한 네온 컬러 토큰 (복구)**: 딥 옵시디언(`0xFF0A0C16`), 아쿠아 글로우(`0xFF00FFCC`), 마젠타 로즈(`0xFFFF0055`) 등 정확한 16진수 ARGB 토큰 및 Glassmorphism 블러 수치(Sigma 20.0, 투명도 0x1A) 복원.
2. **60fps GC-Free 렌더링 최적화 (복구)**: VU 미터 UI의 `setState` 남발로 인한 프레임 드랍을 막기 위해, `CustomPainter` 상에서 `AtomicU32` 값을 읽어 비디오 그래픽 하드웨어 가속 레이어에 네온 그라데이션을 직접 무부하 드로잉한다는 핵심 로직 복원.
3. **환경설정 팝업 모달 스펙 (복구)**: 모달 사이즈 720x560, 드롭다운 디바이스 스캐너 폼 복원.
4. **마이크로 인터랙션 (강화)**: Hover 시 150ms 선형 보간 트랜지션, 장비 플러그 인/아웃 실시간 감지 UI 리프레시 명세 추가.

### [6_TASKS_Milestones.md]
**복구(누락된 원본) 및 강화 사항:**
1. **4단계 오프라인 검증 플랜 (복구)**: GUI 렌더링 락 검증, 로컬 루프백 OSC 모의 패킷 검증, 스마트 더킹 청음 캘리브레이션, 리소스 프로파일링(Activity Monitor) 마일스톤 정식 편입.
2. **48시간 스트레스 테스트 (강화)**: 모의 아두이노 스크립트로 초당 100개의 무작위 패킷을 48시간 주입하여 링버퍼 언더런 0회, OOM 0회를 증명하는 극한 QA 태스크 추가.

### [7_CODING_CONVENTION_Rules.md]
**복구(누락된 원본) 및 강화 사항:**
1. **기능 도메인(Feature-First) 폴더 트리 강제 (복구)**: 원문에 있던 Rust 백엔드 구조(`audio/mixer.rs`, `osc/debouncer.rs` 등)와 Flutter 구조(`features/dashboard`, `core/theme`) 명세를 컨벤션의 표준 아키텍처 규칙으로 박제.
2. **FFI 네이밍 및 에러 타입 매핑 (강화)**: `api_xxx()` 접두사 강제, `AtmosError` 세분화(`AudioDeviceLost`, `NetworkError`) 및 각 에러의 UI 색상(빨강/노랑) 1:1 매핑 규칙 강제.
